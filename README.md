...
```shell
 ...
 git clone https://github.com/leggedrobotics/cmake_clang_tools.git
```
...
wget https://github.com/microsoft/onnxruntime/releases/download/v1.7.0/onnxruntime-linux-${ORT_ARCH}-1.7.0.tgz -O onnxruntime-linux-${ORT_ARCH}-1.7.0.tgz
tar xf onnxruntime-linux-${ORT_ARCH}-1.7.0.tgz
mkdir -p ~/.local/bin ~/.local/include/onnxruntime ~/.local/lib ~/.local/share/cmake/onnxruntime
rsync -a /tmp/onnxruntime-linux-${ORT_ARCH}-1.7.0/include/ ~/.local/include/onnxruntime
rsync -a /tmp/onnxruntime-linux-${ORT_ARCH}-1.7.0/lib/ ~/.local/lib
...
然后在最后一行输入：
```shell
export onnxruntime_DIR=~/.local/share/cmake/onnxruntime
...
cd ~/ocs2_ws/
catkin init
catkin config --cmake-args -DCMAKE_BUILD_TYPE=RelWithDebInfo -Donnxruntime_DIR=~/.local/share/cmake/onnxruntime
...
```shell
# 某些环境（如 WSL/无桌面环境）没有 gnome-terminal，会导致 RLException
# 先把 launch 文件中的 launch-prefix 去掉
sed -i 's|launch-prefix="gnome-terminal --"|launch-prefix=""|g' ~/ocs2_ws/src/ocs2/ocs2_robotic_examples/ocs2_legged_robot_ros/launch/legged_robot_ddp.launch

# 如果没有图形界面，建议先关闭 rviz 验证核心流程
roslaunch ocs2_legged_robot_ros legged_robot_ddp.launch rviz:=false
