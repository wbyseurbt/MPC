...
class LeggedInterface : public RobotInterface {
 public:
  ...
  const ipm::Settings& ipmSettings() const { return ipmSettings_; }

  const vector_t& getInitialState() const { return initialState_; }
  const RolloutBase& getRollout() const { return *rolloutPtr_; }
  PinocchioInterface& getPinocchioInterface() const { return *pinocchioInterfacePtr_; }
  const CentroidalModelInfo& getCentroidalModelInfo() const { return centroidalModelInfo_; }
  const PinocchioGeometryInterface& getGeometryInterface() const { return *geometryInterfacePtr_; }
 ...
 protected:
  ...
  std::unique_ptr<StateInputCost> getSoftSwingTrajConstraint(const EndEffectorKinematics<scalar_t>& eeKinematics, size_t contactPointIndex,
