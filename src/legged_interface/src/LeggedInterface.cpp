...
/******************************************************************************************************/
/******************************************************************************************************/
std::unique_ptr<StateInputCost> LeggedInterface::getSoftSwingTrajConstraint(const EndEffectorKinematics<scalar_t>& eeKinematics,
...
void LeggedInterface::setupReferenceManager(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
  ...
  referenceManagerPtr_ = std::make_shared<SwitchedModelReferenceManager>(
      loadGaitSchedule(referenceFile, verbose), std::move(swingTrajectoryPlanner), *pinocchioInterfacePtr_, centroidalModelInfo_);
}
...
std::unique_ptr<StateInputCost> LeggedInterface::getLimitConstraints(const CentroidalModelInfo& info) {
  ...
  for (int j = 0; j < info.actuatedDofNum; j++) {
    ...
    state_input_limit_penalty[info.actuatedDofNum + j] = std::make_unique<DoubleSidedPenalty>(
        -model.velocityLimit(6 + j), model.velocityLimit(6 + j), std::make_unique<RelaxedBarrierPenalty>(vel_limit_barrier_penalty_config));
  }
  for (int leg = 0; leg < info.numThreeDofContacts; leg++) {
    state_input_limit_penalty[info.actuatedDofNum + info.actuatedDofNum + leg] =
        std::make_unique<DoubleSidedPenalty>(0, 150, std::make_unique<RelaxedBarrierPenalty>(force_limit_barrier_penalty_config));
  }
