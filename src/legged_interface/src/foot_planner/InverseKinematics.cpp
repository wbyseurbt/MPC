...
vector5_t InverseKinematics::computeIK(vector_t init_q, int leg, vector3_t des_foot_linear_xyz, vector3_t des_foot_eular_zyx) {
...
vector5_t InverseKinematics::computeIK(vector_t init_q, int leg, vector3_t des_foot_linear_xyz, matrix3_t des_foot_R_des) {
  const int index = leg2index(leg);
  init_q.segment<5>(6 + index) = computeTranslationIK(init_q, leg, des_foot_linear_xyz);
...
vector5_t InverseKinematics::computeTranslationIK(vector_t init_q, int leg, vector3_t des_foot_linear_xyz) {
  ...
  if (last_err_norm < err_tol) {
    if (verbose) printf("[translation IK] small err at init, err norm: %f\n", last_err_norm);
  } else {
    int i = 0;
    while (true) {
      ...
      if (err_norm > last_err_norm) {
        if (verbose)
          printf(
              "[translation IK] local min at %d, last err norm: %f, new err "
              "norm: %f\n",
              i, last_err_norm, err_norm);
        break;
      }
      if (abs(err_norm - last_err_norm) < converage_tol) {
        if (verbose) printf("[translation IK] converage at %d, new err norm: %f\n", i, err_norm);
        break;
      }
      ...
      if (err_norm < err_tol) {
        if (verbose) printf("[translation IK] small err at %d, err norm: %f\n", i, err_norm);
        break;
      }
      i++;
      if (i >= max_it) {
        if (verbose) printf("[translation IK] max iter at %d, err norm: %f\n", i, err_norm);
        break;
      }
    }
  }
...
vector5_t InverseKinematics::computeRotationIK(vector_t init_q, int leg, matrix3_t des_foot_R_des) {
  ...
  if (last_err_norm < err_tol) {
    if (verbose) printf("[rotation IK] small err at init, err norm: %f\n", last_err_norm);
  } else {
    int i = 0;
    while (true) {
      ...
      if (err_norm > last_err_norm) {
        if (verbose)
          printf(
              "[rotation IK] local min at %d, last err norm: %f, new err "
              "norm: %f\n",
              i, last_err_norm, err_norm);
        break;
      }
      if (abs(err_norm - last_err_norm) < converage_tol) {
        if (verbose)
          printf(
              "[rotation IK] converage at %d, last err norm: %f, new err "
              "norm: %f\n",
              i, last_err_norm, err_norm);
        break;
      }
      ...
      if (err_norm < err_tol) {
        if (verbose) printf("[rotation IK] small err at %d, err norm: %f\n", i, err_norm);
        break;
      }
      i++;
      if (i >= max_it) {
        if (verbose) printf("[rotation IK] max iter at %d, err norm: %f\n", i, err_norm);
        break;
      }
    }
  }
