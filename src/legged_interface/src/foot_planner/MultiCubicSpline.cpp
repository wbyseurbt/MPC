...
scalar_t MultiCubicSpline::position(scalar_t time) const {
  ...
  if (numerics::almost_eq(time, nodes_.back().time)) return cubic_splines_.back().position(time);
  if (time < nodes_.front().time) return cubic_splines_.front().position(time);
  if (time > nodes_.back().time) return cubic_splines_.back().position(time);
  return 0;
}
...
scalar_t MultiCubicSpline::velocity(scalar_t time) const {
  ...
  if (numerics::almost_eq(time, nodes_.back().time)) return cubic_splines_.back().velocity(time);
  if (time < nodes_.front().time) return cubic_splines_.front().velocity(time);
  if (time > nodes_.back().time) return cubic_splines_.back().velocity(time);
  return 0;
}
...
scalar_t MultiCubicSpline::acceleration(scalar_t time) const {
  ...
  if (numerics::almost_eq(time, nodes_.back().time)) return cubic_splines_.back().acceleration(time);
  if (time < nodes_.front().time) return cubic_splines_.front().acceleration(time);
  if (time > nodes_.back().time) return cubic_splines_.back().acceleration(time);
  return 0;
}
