...
class WbcBase {
 ...
 public:
  ...
  void setEarlyLateContact(const std::array<contact_flag_t, 2>& early_late_contact) { earlyLatecontact_ = early_late_contact; }

  void setFootPosVelAccDesired(const std::array<vector_t, 3>& footPosVelAccDesired) { footPosVelAccDesired_ = footPosVelAccDesired; }
  void setJointAccDesired(const vector_t& jointAccDesired) { jointAccDesired_ = jointAccDesired; }
  void setKpKd(scalar_t swingKp, scalar_t swingKd) {
    swingKp_ = swingKp;
    swingKd_ = swingKd;
  }
  size_t getContactForceSize() { return contact_force_size_; }
  void setStanceMode(bool stance_mode) { stance_mode_ = stance_mode; }
  matrix_t getContactJacobi() { return j_; }

 protected:
  void updateMeasured(const vector_t& rbdStateMeasured);
  void updateDesired(const vector_t& stateDesired, const vector_t& inputDesired);

  size_t getNumDecisionVars() const {  // std::cout<<"numDecisionVars_:  "<<numDecisionVars_<<std::endl;//输出38
