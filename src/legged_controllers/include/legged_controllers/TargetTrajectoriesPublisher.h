...
class TargetTrajectoriesPublisher final {
 public:
  ...
  TargetTrajectoriesPublisher(::ros::NodeHandle& nh, const std::string& topicPrefix, CmdToTargetTrajectories goalToTargetTrajectories,
                              CmdToTargetTrajectories cmdVelToTargetTrajectories, CmdToTargetTrajectories cmdPosToTargetTrajectories)
    ...
    auto goalCallback = [this](const geometry_msgs::PoseStamped::ConstPtr& msg) {
      std::cout << "received goal" << msg->pose.position.x << " " << msg->pose.position.y << " " << msg->pose.position.z << std::endl
 ...
 private:
  ...
  ::ros::Subscriber observationSub_, goalSub_, cmdVelSub_, comAdjustmentSub_, runningSwitchsub_, stanceSwitchsub_, bodyRotationsub_,
      bodyrotationSwitchsub_, pawupSwitchsub_;
