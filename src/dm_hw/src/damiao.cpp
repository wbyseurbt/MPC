...
{
    // 先对所有电机清除故障（0xFB），再发使能（0xFC）
    std::set<Motor_id> unique_slave_ids;
    for (auto& it : motors)
        unique_slave_ids.insert(it.second->GetSlaveId());

    std::cerr << "[Motor_Control] Clearing faults for " << unique_slave_ids.size()
              << " motor(s): ";
    for (auto id : unique_slave_ids) std::cerr << id << " ";
    std::cerr << std::endl;

    for (auto id : unique_slave_ids)
    {
        control_cmd(id, 0xFB);   // clear fault
        usleep(2000);
    }
    usleep(100000);  // 100ms wait after fault clear

    std::cerr << "[Motor_Control] Enabling motors..." << std::endl;
    for (auto id : unique_slave_ids)
    {
        for (int j = 0; j < 20; j++)
        {
        ...
        std::cerr << "  -> enabled CAN ID " << id << std::endl;
    }
    std::cerr << "[Motor_Control] Enable sequence done." << std::endl;
}
...
void Motor_Control::write()
{
    ...
    bool do_print = (++write_count % 1000 == 1);  // print once every 1000 cycles (~2s at 500Hz)

    for(const auto& m : *data_ptr_)
    {
        int motor_id = m.first;
        ...
        if (do_print)
        {
            std::cerr << "[write] CAN_ID=" << motor_id
                      << "  kp=" << m.second.kp
                      << "  kd=" << m.second.kd
                      << "  pos=" << m.second.cmd_pos
                      << "  vel=" << m.second.cmd_vel
                      << "  ff=" << m.second.cmd_effort
                      << std::endl;
        }
